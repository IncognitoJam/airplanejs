...
function airportMarkerClick() {
  ...
  airportInfoWindow.setContent(`
    ...
    <table class="list">
      ...
      <tr><th>Altitude</th><td>${
        airport.altitude === null ? "n/a" : airport.altitude + " ft"
      }</td></tr>
    </table>
  `);
...
function plotAircrafts(aircrafts) {
  aircrafts.forEach(function(_aircraft) {
    const aircraft = (aircraftIndex[_aircraft.icao] =
      aircraftIndex[_aircraft.icao] || new Aircraft());
...
Aircraft.prototype.toHTML = function() {
  ...
  const altitude = Number.isFinite(this.altitude)
    ? this.altitude + " " + unit
    : "Unknown";
